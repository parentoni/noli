...
describe("Find user by token", () => {
  it("Should decrypt token and return user", async () => {
    ...
    const token = authService.signToken({id: user._id, email: user.email, tokenType: TOKEN_TYPES.AUTHENTICATE})
  ...
  it("Should return Invalid error when user is not found", async () => {
    const token = authService.signToken({id: 'invalid', email: 'invalid@invalid.com', tokenType: TOKEN_TYPES.AUTHENTICATE})
    const userByToken = await getUserByTokenUseCase.execute({token: token})
  ...
  it("Should return UnexpectedError when an error occurs", async () => {
    const token = authService.signToken({id: 'error', email: 'error@error.com', tokenType: TOKEN_TYPES.AUTHENTICATE})
    const userByToken = await getUserByTokenUseCase.execute({token: token})
  ...
  it("Should return Invalid error when token type is not authenticate", async () => {
    const token = authService.signToken({id: 'teste', email: 'error@error.com', tokenType: TOKEN_TYPES.CHANGE_PASSWORD})
    const userByToken = await getUserByTokenUseCase.execute({token: token})
